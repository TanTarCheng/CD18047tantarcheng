...
#put your hand on head and pull to left hand side
...
def calculate_post(img):
    ...
    if len(lmlist) != 0:
        angle = detector.findAngle(img, 15, 11, 23)
        # range (210 -310) convert to 0 - 100 percent
        per = np.interp(angle, (190, 210), (0, 100))
        # 650= min bar , 100 = max bar opencv is oppesite de
        bar = np.interp(angle, (190, 210), (650, 100))
